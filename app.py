...
def add_watermark(image_bytes: bytes, is_premium: bool = False, watermark_text: str = "DreamAI") -> bytes:
    ...
    try:
        ...
        try:
            ...
            center_font = None
            if not is_premium:
                center_font_size = min(int(img_height * 0.8), 2000)  # Max 80% height or 2000px
                print(f"[WATERMARK DEBUG] Image size: {img_width}x{img_height}")
                print(f"[WATERMARK DEBUG] Center font calc: min({int(img_height * 0.8)}, 2000) = {center_font_size}")
                for font_path in font_paths:
                    if os.path.exists(font_path):
                        center_font = ImageFont.truetype(font_path, center_font_size)
                        print(f"[WATERMARK DEBUG] Center font loaded from: {font_path}")
                        break
                if center_font is None:
                    center_font = ImageFont.load_default()
                    print(f"[WATERMARK DEBUG] WARNING: Center font using default (size parameter ignored!)")

            # Bottom right watermark - much larger size
            bottom_font_size = min(int(img_height * 0.30), 800)  # Max 30% height or 800px
            print(f"[WATERMARK DEBUG] Bottom font calc: min({int(img_height * 0.30)}, 800) = {bottom_font_size}")
            bottom_font = None
            for font_path in font_paths:
                if os.path.exists(font_path):
                    bottom_font = ImageFont.truetype(font_path, bottom_font_size)
                    print(f"[WATERMARK DEBUG] Bottom font loaded from: {font_path}")
                    break
            if bottom_font is None:
                bottom_font = ImageFont.load_default()
                print(f"[WATERMARK DEBUG] WARNING: Bottom font using default (size parameter ignored!)")
        ...
        if not is_premium:
            ...
            # Draw with 60% opacity (alpha = 153 out of 255) with subtle stroke for better quality
            overlay_draw.text((center_x, center_y), center_text, fill=(255, 255, 255, 153), font=center_font, stroke_width=4, stroke_fill=(0, 0, 0, 100))
        ...
        # Draw text with 100% opacity (white text, no background) with subtle stroke for better quality
        overlay_draw.text((bottom_x, bottom_y), bottom_text, fill=(255, 255, 255, 255), font=bottom_font, stroke_width=3, stroke_fill=(0, 0, 0, 120))
