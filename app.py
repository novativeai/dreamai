...
def add_watermark(image_bytes: bytes, is_premium: bool = False, watermark_text: str = "DreamAI") -> bytes:
    ...
    Free users: Double watermark
      - "DreamAI" centered at 5% opacity
      - "AI Generated by DreamAI" in bottom right at 100% opacity
    ...
    try:
        ...
        try:
            ...
            center_font = None
            if not is_premium:
                center_font_size = max(int(img_height * 0.1), 60)  # 10% of height, min 60px
            ...
            bottom_font_size = max(int(img_height * 0.025), 12)  # 2.5% of height, min 12px
            bottom_font = None
            for font_path in font_paths:
                if os.path.exists(font_path):
        ...
        # FREE USERS: Add center watermark "DreamAI" at 5% opacity
        if not is_premium:
            center_text = "DreamAI"
            ...
            # Center position
            center_x = (img_width - center_text_width) // 2
            center_y = (img_height - center_text_height) // 2

            # Draw with 5% opacity (alpha = 13 out of 255)
            overlay_draw.text((center_x, center_y), center_text, fill=(255, 255, 255, 13), font=center_font)
        ...
        padding = int(img_height * 0.02)  # 2% padding
        bottom_x = img_width - bottom_text_width - padding
        bottom_y = img_height - bottom_text_height - padding

        # Draw background rectangle for better visibility (semi-transparent black)
        background_padding = 4
        overlay_draw.rectangle(
            [
                (bottom_x - background_padding, bottom_y - background_padding),
                (bottom_x + bottom_text_width + background_padding, bottom_y + bottom_text_height + background_padding)
            ],
            fill=(0, 0, 0, 153)  # 60% opacity black background
        )

        # Draw text with 100% opacity
