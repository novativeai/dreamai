...
def add_watermark(image_bytes: bytes, is_premium: bool = False, watermark_text: str = "DreamAI") -> bytes:
    ...
    try:
        ...
        try:
            ...
            center_font = None
            if not is_premium:
                center_font_size = 500  # Fixed large size
                print(f"[WATERMARK DEBUG] Image size: {img_width}x{img_height}")
                print(f"[WATERMARK DEBUG] Center font size: {center_font_size}px")
            ...
            # Bottom right watermark - fixed large size
            bottom_font_size = 250  # Fixed large size
            print(f"[WATERMARK DEBUG] Bottom font size: {bottom_font_size}px")
            bottom_font = None
            for font_path in font_paths:
                if os.path.exists(font_path):
