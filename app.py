...
def add_watermark(image_bytes: bytes, is_premium: bool = False, watermark_text: str = "DreamAI") -> bytes:
    ...
    try:
        ...
        try:
            ...
            center_font = None
            if not is_premium:
                center_font_size = min(int(img_height * 0.08), int(img_width * 0.2), 100)  # Max 8% height, 20% width, or 100px
            ...
            bottom_font_size = min(int(img_height * 0.02), int(img_width * 0.04), 20)  # Max 2% height, 4% width, or 20px
            bottom_font = None
            for font_path in font_paths:
                if os.path.exists(font_path):
        ...
        if not is_premium:
            ...
            # Draw with 20% opacity (alpha = 51 out of 255)
            overlay_draw.text((center_x, center_y), center_text, fill=(255, 255, 255, 51), font=center_font)
